...
spinButton.addEventListener("click", async () => {
  ...
  try {
    ...
    const w1 = await wheel1.spin();
    const w2 = await wheel2.spin();
    const w3 = await wheel3.spin();
    Promise.all([w1, w2, w3]).then((values) => {
      console.log(values);
      const symbol = getMaxCount(values);
      console.log(symbol);
      if (symbol == 3) {
        result.setResult(100);
      } else if (symbol == 2) {
        result.setResult(10);
      } else result.setResult(0);
    });
  } catch {
